...
public class BaseTable {
    ...
    BaseTable(String tableName) throws SQLException { // Для реальной таблицы передадим в конструктор её имя
        ...
        this.connection = SharesDBManipulator.getConnection(); // Установим соединение с СУБД для дальнейшей работы
    }
    ...
    // Выполнить SQL команду (Statement) без параметров в СУБД, по завершению выдать сообщение в консоль
    ...
    void reopenConnection() throws SQLException {
        if (connection == null || connection.isClosed()) {
            connection = SharesDBManipulator.getConnection();
        }
    }
